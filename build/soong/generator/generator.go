...
func init() {
	android.RegisterModuleType("wrath_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = wrathExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = wrathExpandVariables(ctx, dep_file)
	...
	cmd := wrathExpandVariables(ctx, String(g.properties.Cmd))
